...
def _create_minimal_unit_test_csv(target_dir):
    """Creates minimal synthetic rows for pytest execution inside test_data_tmp."""
    for yr in [2021, 2022, 2023]:
        dates = pd.date_range(f"{yr}-01-01", periods=150, freq="3h")
        temp = np.random.uniform(20, 35, len(dates))
        solar = np.random.uniform(5, 25, len(dates))
        sm = np.random.uniform(0.1, 0.4, len(dates))
        wind = np.random.uniform(1, 5, len(dates))
        et = 0.08 * temp + 0.12 * solar + 4.0 * sm + 0.05 * wind + np.random.normal(0, 0.05, len(dates))
        ...
        df = pd.DataFrame({
            ...
            'temp_c': temp,
            'wind_speed': wind,
            'pressure_kpa': np.random.uniform(98, 101, len(dates)),
            'solar_rad': solar,
            'precip': np.random.uniform(0, 10, len(dates)),
            'soil_moisture': sm,
            'ndvi': np.random.uniform(0.3, 0.8, len(dates)),
            'modis_et_mm': et
        })
        df.to_csv(os.path.join(target_dir, f"cwf_epoch_{yr}.csv"), index=False)
...
def setup_teardown_test_env():
    ...
    _create_minimal_unit_test_csv(TEST_DATA_DIR)
    compile_datasets(data_dir=TEST_DATA_DIR)
    yield
...
def test_trainer_execution():
    ...
    assert len(best_run['weight_adjustments']) in [len(FEATURES), len(getattr(__import__('config'), 'EXTENDED_FEATURES', []))], "Feature weight count mismatch."
    ...
    assert prod_run['global_r2_accuracy'] > 0.80, "Production model accuracy is below threshold."
...
def test_adaptive_self_training_and_hot_reloading():
    ...
    trainer = AdaptiveModelTrainer(data_dir=TEST_DATA_DIR, output_dir=TEST_OUTPUT_DIR)
    df = trainer.load_master_dataset().sample(min(200, len(trainer.load_master_dataset())), random_state=42)
